...
const videoRegex = /https?:\/\/(?:www\.)?(?:facebook\.com\/(?:watch\/\?v=|username\/videos\/|video.php\?v=|reel\/|share\/(?:r|v)\/|videos\/\w+)|instagram\.com\/(?:reel\/|p\/|USERNAME\/media\/))([\w-]+)/i;
...
async function downloadVideo(message, videoUrl) {
    ...
    const updateLoadingMessage = setInterval(async () => {
        loadingDots = (loadingDots + 1) % 4; // Cycle through 0 to 3
    ...
    try {
        ...
        writer.on('finish', async () => {
            clearInterval(updateLoadingMessage); // Stop loading message updates
            ...
            if (fileSizeInMB > thresholdSize) {
                // Start updating loading message for compression
                loadingDots = 0;
